...
#elif defined(__APPLE__)
    // Use macOS native dispatch semaphore (more efficient than condition variables)
    _autoResetEvent = dispatch_semaphore_create(0);
#else
    _autoResetEvent = new EventWrapper();
#endif
...
#elif defined(__APPLE__)
    dispatch_release((dispatch_semaphore_t)_autoResetEvent);
#else
    delete static_cast<EventWrapper*>(_autoResetEvent);
#endif
}
...
QString PeakCanInterface::getDescription() const {
    return  QString("pcan usb") ;
}
...
#elif defined(__APPLE__)
    // Use macOS native dispatch_semaphore for efficient waiting
    dispatch_time_t timeout = dispatch_time(DISPATCH_TIME_NOW, timeout_ms * NSEC_PER_MSEC);
    if (dispatch_semaphore_wait((dispatch_semaphore_t)_autoResetEvent, timeout) != 0) {
        // Timeout - still try to read
    }
#else
    // On Linux, PCAN lib doesn't support events, so we use condition variable
    EventWrapper* event = static_cast<EventWrapper*>(_autoResetEvent);
    std::unique_lock<std::mutex> lock(event->mutex);
    if (!event->cv.wait_for(lock, std::chrono::milliseconds(timeout_ms), [event]{ return event->signaled; })) {
        // Timeout occurred, try reading anyway
    }
